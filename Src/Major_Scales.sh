...
note7=$9
note8=${10}
cat > ./files/${title}_Major_Scales.ly << EOF
\version "2.19.82"
...
\markup{"$title Major Scale (Two Octaves)"}
\score {{
    \global
    \relative c' {
...
\markup{"$title Major Pentatonic Scale (Lower Octave)"}
\score {{
    \global
    \relative c' {
...
\markup{"$title Major Pentatonic Scale (Higher Octave)"}
\score {{
    \global
    \relative c'' {
      	$note1 4 $note2 $note3  $note5 $note6  $note8 $note8  $note6 $note5  $note3 $note2 $note1
...
\markup{"$title Major Pentatonic Scale (Two Octave)"}
\score {{
    \global
    \relative c' {
